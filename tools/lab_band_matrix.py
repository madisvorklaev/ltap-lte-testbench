#!/usr/bin/env python3
"""Durable controlled LTE band matrix runner for the LtAP public iPerf lab."""
...
CAMPAIGN_ID = "band-matrix-7.24rc3"
RUNTIME = REPO / "runtime" / CAMPAIGN_ID
PUBLIC = REPO / "results-public" / CAMPAIGN_ID
STOP_FILE = REPO / "runtime/STOP_BAND_MATRIX"
BRANCH = "band-matrix-7.24rc3"
...
def items() -> list[dict[str, Any]]:
    ...
    for round_name, seq in (("R1", primary), ("R2", list(reversed(primary)))):
        for code, lte1, lte2 in seq:
            out.append({"id": f"{round_name}-{code}", "round": round_name, "matrix": code, "lte1_band": lte1, "lte2_band": lte2, "required": True})
    for code, lte1, lte2 in coverage:
        out.append({"id": code, "round": "coverage", "matrix": code, "lte1_band": lte1, "lte2_band": lte2, "required": True})
    return out
...
class Runner:
    ...
    def run_dual(self, item: dict[str, Any], attempt: int) -> tuple[bool, dict[str, Any]]:
        ...
        commands = [
            base + ["--campaign", str(CAMPAIGN_LTE1), "--path", "lte1", "--protocol", "udp", "--bitrate", "6M", "--packet-length", "1200", "--duration", "300", "--warmup", "10", "--cooldown", "10", "--allow-concurrent-other-lte", "--tag", f"{tag}_lte1", "--output", str(raw_root)],
            base + ["--campaign", str(CAMPAIGN_LTE2), "--path", "lte2", "--protocol", "udp", "--bitrate", "6M", "--packet-length", "1200", "--duration", "300", "--warmup", "10", "--cooldown", "10", "--allow-concurrent-other-lte", "--tag", f"{tag}_lte2", "--output", str(raw_root)],
        ]
        procs = [
    ...
    def git_checkpoint(self, message: str) -> None:
        paths = ["tools/lab_band_matrix.py", "results-public/band-matrix-7.24rc3"]
    ...
    def run(self) -> None:
        ...
        try:
            ...
            self.save_progress()
            self.git_checkpoint("band-matrix: initialize progress")
            for item in items():
    ...
    def run_item(self, item: dict[str, Any]) -> None:
        ...
        try:
            ...
            self.git_checkpoint(f"band-matrix: {terminal.lower()} {item_id}")
