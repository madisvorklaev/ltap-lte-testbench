#!/usr/bin/env python3
"""dual-operator LTE7 SIM crossover campaign."""
...
base = quick.base
CAMPAIGN_ID = "lte7-elisa-telia-crossover"
BRANCH = "lte7-elisa-telia-crossover"
TARGET_VERSION = "7.24rc3"
EXPECTED_LTE1_MODEL = "R11l-LTE7"
EXPECTED_LTE2_MODEL = "R11l-LTE7"
EXPECTED_LTE1_FIRMWARE = "R11l-LTE7_V005"
EXPECTED_LTE2_FIRMWARE = "R11l-LTE7_V005"

REPO = base.REPO
RUNTIME = REPO / "runtime" / CAMPAIGN_ID
PUBLIC = REPO / "results-public" / CAMPAIGN_ID
STOP_FILE = REPO / "runtime/STOP_LTE7_ELISA_TELIA_CROSSOVER"
...
CANDIDATE_BANDS = ["1", "3", "7", "8", "20", "28", "38"]
SELECTION_PRIORITY = ["3", "7", "20", "8"]
OPERATOR_ALIASES = {
    "elisa": "elisa",
    "elisa eesti": "elisa",
    "telia": "telia",
...
def modem_id_from(identity: dict[str, Any]) -> str:
    source = (
        identity.get("imei")
        or identity.get("serial-number")
        or identity.get("software-id")
        or f"{identity.get('interface')}:{identity.get('model')}:{identity.get('revision')}"
    )
    digest = hashlib.sha256(str(source).encode("utf-8")).hexdigest()[:10]
...
def current_items(progress: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    ...
    if progress:
        ...
        if progress.get("include_auto_tests") and "" not in selected:
            selected.append("")
    ...
    for phase in ("A", "B"):
        for band in selected:
            ...
            if band == "7":
                for confirm in (progress or {}).get("b7_confirmations", []):
                    if confirm.get("phase") == phase:
                        out.append({
                            "id": confirm["id"],
                            "phase": f"PHASE_{phase}_LOADED",
                            "crossover_phase": phase,
...
def staged_secret_hits() -> list[str]:
    ...
    return [
        ...
        if not line.startswith("tools/lte7_elisa_telia_crossover.py:")
    ]


class Runner(quick.Runner):
    def public_modem_entry(self, iface: str, modem: dict[str, Any]) -> dict[str, Any]:
        return {
            "interface": iface,
            "public_label": modem.get("public_label"),
            "modem_id": modem.get("modem_id"),
            "model": modem.get("detected"),
    ...
    def verify_baseline(self) -> bool:
        ...
        self.progress["modem_lte1"] = {
            "expected": EXPECTED_LTE1_MODEL,
            "detected": lte1_model,
            "firmware": lte1.get("revision"),
            "public_label": "LTE7-A",
            "modem_id": modem_id_from({"interface": "lte1", **lte1, "model": lte1_model}),
        }
        self.progress["modem_lte2"] = {
            "expected": EXPECTED_LTE2_MODEL,
            "detected": lte2_model,
            "firmware": lte2.get("revision"),
            "public_label": "LTE7-B",
            "modem_id": modem_id_from({"interface": "lte2", **lte2, "model": lte2_model}),
        }
        self.progress["modem_map"] = {
            "collected_at": utcnow(),
            "interfaces": {
                "lte1": self.public_modem_entry("lte1", self.progress["modem_lte1"]),
                "lte2": self.public_modem_entry("lte2", self.progress["modem_lte2"]),
            },
        }
        base.atomic_write_json(PUBLIC / "MODEM_MAP_PUBLIC.json", self.progress["modem_map"])
        ...
        lte1_ok = re.search(r"R11l-LTE7|LTE7", str(lte1_model), re.I) is not None
        lte2_ok = re.search(r"R11l-LTE7|LTE7", str(lte2_model), re.I) is not None
        fw1 = lte1.get("revision")
        fw2 = lte2.get("revision")
        firmware_symmetric = bool(fw1 and fw2 and fw1 == fw2)
        ...
        self.save_progress()
        if all([routeros_ok, routerboard_ok, lte1_ok, lte2_ok, firmware_symmetric]):
            return True
        self.progress["state"] = "BLOCKED_LTE7_FIRMWARE_ASYMMETRY" if lte1_ok and lte2_ok and not firmware_symmetric else "BLOCKED_ROUTER_BASELINE_MISMATCH"
        self.last_error = (
            f"{self.progress['state']}: "
    ...
    def public_sim_entry(self, identity: dict[str, Any]) -> dict[str, Any]:
        modem = self.progress.get(f"modem_{identity.get('interface')}", {})
        return {
            "interface": identity.get("interface"),
            "modem": identity.get("modem"),
            "modem_label": modem.get("public_label"),
            "modem_id": modem.get("modem_id"),
    ...
    def collect_sim_map(self, phase_name: str) -> dict[str, Any]:
        ...
        if phase_name == "PHASE_A":
            base.atomic_write_json(PUBLIC / "PHASE_A_SIM_MAP.json", public)
        elif phase_name == "PHASE_B":
            base.atomic_write_json(PUBLIC / "PHASE_B_SIM_MAP.json", public)
        return public
    ...
    def run_discovery(self) -> None:
        ...
        for band_key, result in telia["bands"].items():
            ...
            if result.get("status") != "AVAILABLE":
                common["bands"][band_key] = {"telia": False, "elisa": False, "selected": False, "reason": "Telia unavailable"}
                continue
            elisa_result = self.discover_band(elisa_iface, band)
            elisa["bands"][band_key] = elisa_result
            base.atomic_write_json(PUBLIC / "ELISA_BAND_DISCOVERY.json", base.sanitize_obj(elisa))
            ...
            self.bump_worker_progress()
            self.save_progress()
        elisa["finished_at"] = utcnow()
        base.atomic_write_json(PUBLIC / "ELISA_BAND_DISCOVERY.json", base.sanitize_obj(elisa))
        self.progress["elisa_band_discovery"] = elisa
        selected: list[str] = []
        for band in SELECTION_PRIORITY:
            entry = common["bands"].get(f"B{band}")
            if entry and entry.get("telia") and entry.get("elisa"):
        ...
        self.progress["include_auto_tests"] = True
        ...
        base.atomic_write_json(PUBLIC / "COMMON_BANDS.json", base.sanitize_obj(common))
        self.update_matrix_summary()
        self.git_checkpoint("lte7 crossover: discovery complete")
    ...
    def run_loaded_item(self, item: dict[str, Any]) -> None:
        ...
        try:
            ...
            if not verified:
                ...
                self.update_matrix_summary()
                self.git_checkpoint(f"lte7 crossover: {item_id_} unavailable")
                return
            ...
            while attempts < 2:
                ...
                if ok:
                    ...
                    self.maybe_enqueue_b7_confirmation(item, data)
                    self.update_matrix_summary()
                    self.git_checkpoint(f"lte7 crossover: complete {item_id_}")
                    return
            ...
            self.maybe_enqueue_b7_confirmation(item, data)
            self.update_matrix_summary()
            self.git_checkpoint(f"lte7 crossover: failed {item_id_}")
        except Exception as exc:
            ...
            self.update_matrix_summary()
            self.git_checkpoint(f"lte7 crossover: {terminal.lower()} {item_id_}")
    ...
    def maybe_enqueue_b7_confirmation(self, item: dict[str, Any], data: dict[str, Any]) -> None:
        if item.get("confirmation") or item.get("lte1_band") != "7" or item.get("lte2_band") != "7":
            return
        phase = item.get("crossover_phase")
        if not phase:
            return
        ...
        def stats(path_name: str) -> dict[str, Any]:
            ps = data.get(path_name) or {}
            return {"loss": (ps.get("iperf") or {}).get("lost_percent"), "p95": (ps.get("ping") or {}).get("p95_ms"), "mbps": (ps.get("iperf") or {}).get("mbps")}

        def poor(x: dict[str, Any]) -> bool:
        ...
        lte1 = stats("lte1")
        lte2 = stats("lte2")
        if (poor(lte1) and good(lte2)) or (poor(lte2) and good(lte1)):
    ...
    def wait_for_swap(self) -> bool:
        ...
        self.save_progress()
        self.update_matrix_summary()
        self.write_action_required()
        self.git_checkpoint("lte7 crossover: waiting for physical sim swap")
        ...
        while not STOP_FILE.exists():
            try:
                ...
                modem_ok = self.modems_still_on_same_interfaces()
                if reversed_ok and modem_ok and self.validate_one_elisa_one_telia(after):
                    ...
                    self.last_error = ""
                    self.write_swap_verified()
                    self.save_progress()
                    self.update_matrix_summary()
                    self.git_checkpoint("lte7 crossover: sim swap verified")
                    return True
                if not modem_ok:
                    self.progress["state"] = "BLOCKED_MODEM_MOVED_DURING_SWAP"
                    self.last_error = "BLOCKED_MODEM_MOVED_DURING_SWAP"
                    self.save_progress()
                    self.update_matrix_summary()
                    self.git_checkpoint("lte7 crossover: modem moved during swap")
                    return False
                ...
                self.last_error = repr(exc)
                self.save_progress()
            print("[ACTION REQUIRED] Waiting for Elisa/Telia SIM swap between LTE7-A and LTE7-B.", flush=True)
            time.sleep(30)
        return False

    def modems_still_on_same_interfaces(self) -> bool:
        before = self.progress.get("modem_map", {}).get("interfaces", {})
        for iface in ("lte1", "lte2"):
            mon = self.monitor(iface)
            modem = self.progress.setdefault(f"modem_{iface}", {})
            modem["detected"] = mon.get("model") or modem.get("detected")
            modem["firmware"] = mon.get("revision") or modem.get("firmware")
            modem["modem_id"] = modem_id_from({"interface": iface, **mon, "model": modem.get("detected")})
        current = {
            "lte1": self.public_modem_entry("lte1", self.progress.get("modem_lte1", {})),
            "lte2": self.public_modem_entry("lte2", self.progress.get("modem_lte2", {})),
        }
        for iface in ("lte1", "lte2"):
            if before.get(iface, {}).get("modem_id") != current.get(iface, {}).get("modem_id"):
                return False
        return True

    def write_action_required(self) -> None:
        phase_a = self.progress.get("phase_a_sim_map") or {}
        lte1 = (phase_a.get("interfaces") or {}).get("lte1", {})
        lte2 = (phase_a.get("interfaces") or {}).get("lte2", {})
        lines = [
            "# ACTION REQUIRED - SWAP THE TWO SIM CARDS NOW",
            "",
            "Phase A is complete.",
            "",
            "Swap ONLY the Elisa and Telia SIM cards between the two LTE7 modems.",
            "",
            "DO NOT move:",
            "- LTE7 modems",
            "- pigtails",
            "- antennas",
            "- MAIN/AUX connections",
            "- any other cables",
            "",
            f"Before: LTE1 / LTE7-A: {lte1.get('operator')} / {lte1.get('sim_id')}",
            f"Before: LTE2 / LTE7-B: {lte2.get('operator')} / {lte2.get('sim_id')}",
            "",
            "Required after swap:",
            "LTE1 / LTE7-A: opposite SIM/operator",
            "LTE2 / LTE7-B: opposite SIM/operator",
            "",
            "OpenClaw is waiting and will continue automatically after it verifies the crossover.",
        ]
        (PUBLIC / "ACTION_REQUIRED.md").write_text("\n".join(lines) + "\n", encoding="utf-8")

    def write_swap_verified(self) -> None:
        action = PUBLIC / "ACTION_REQUIRED.md"
        if action.exists():
            action.unlink()
        lines = [
            "# SIM SWAP VERIFIED",
            "",
            f"Verified at: {utcnow()}",
            "",
            "LTE7-A and LTE7-B stayed on the same interfaces.",
            "Elisa and Telia SIM pseudonymous IDs moved to the opposite LTE7 modem.",
        ]
        (PUBLIC / "SIM_SWAP_VERIFIED.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    ...
    def update_matrix_summary(self) -> None:
        ...
        for item in current_items(self.progress):
            ...
            for path_name in ("lte1", "lte2"):
                ...
                modem = self.progress.get(f"modem_{path_name}", {})
                row[f"{path_name}_modem"] = modem.get("public_label") or modem.get("detected")
                row[f"{path_name}_modem_model"] = modem.get("detected")
    ...
    def classification(self) -> str:
        if not self.progress.get("sim_swap_verified"):
            return "LTE7_INCONCLUSIVE"
        rows = base.load_json(PUBLIC / "matrix_summary.json", {}).get("items", {})
        b7: dict[tuple[str, str], dict[str, Any]] = {}
        ...
        b_telia = b7.get(("LTE7-B", "telia"))
        if poor(a_elisa) and poor(b_elisa) and good(a_telia) and good(b_telia):
            return "ELISA_B7_OPERATOR_RAN_EFFECT"
        if poor(a_telia) and poor(b_telia) and good(a_elisa) and good(b_elisa):
            return "TELIA_B7_OPERATOR_RAN_EFFECT"
        if poor(a_elisa) and poor(a_telia) and good(b_elisa) and good(b_telia):
            return "LTE7_MODEM_UNIT_SPECIFIC_B7"
        if poor(b_elisa) and poor(b_telia) and good(a_elisa) and good(a_telia):
            return "LTE7_MODEM_UNIT_SPECIFIC_B7"
        if all(good(x) for x in (a_elisa, a_telia, b_elisa, b_telia) if x):
            return "LTE7_STRONG_DUAL_OPERATOR_CANDIDATE"
        return "LTE7_INCONCLUSIVE"
    ...
    def write_report(self, summary_obj: dict[str, Any]) -> None:
        ...
        lines = [
            f"# dual-operator LTE7 crossover {CAMPAIGN_ID}",
            "",
    ...
    def write_status(self, summary_obj: dict[str, Any]) -> None:
        sim_map = self.progress.get("phase_b_sim_map") if self.progress.get("phase", "").startswith("PHASE_B") else self.progress.get("phase_a_sim_map")
        interfaces = (sim_map or {}).get("interfaces", {})
        item = item_map().get(self.progress.get("current_item") or "", {})
        lte1_band = item.get("lte1_band")
        lte2_band = item.get("lte2_band")
        lines = [
            "ACTION REQUIRED: SWAP ELISA AND TELIA SIM CARDS NOW" if self.progress.get("state") == "WAIT_PHYSICAL_SIM_SWAP" else f"# Status {CAMPAIGN_ID}",
            "",
            ...
            f"- Current phase: {self.progress.get('phase')}",
            f"- LTE1: LTE7-A / {(interfaces.get('lte1') or {}).get('operator') or '-'} / {('AUTO' if lte1_band == '' else 'B' + lte1_band) if lte1_band is not None else '-'}",
            f"- LTE2: LTE7-B / {(interfaces.get('lte2') or {}).get('operator') or '-'} / {('AUTO' if lte2_band == '' else 'B' + lte2_band) if lte2_band is not None else '-'}",
            ...
            f"- Last completed result: {self.last_completed_result(summary_obj)}",
            f"- Next action: {self.next_action()}",
            f"- Last heartbeat: {utcnow()}",
    ...
    def last_completed_result(self, summary_obj: dict[str, Any]) -> str:
        for row in reversed(list(summary_obj.get("items", {}).values())):
            if row.get("state") in base.TERMINAL_ITEM_STATES:
                return f"{row.get('item_id')} {row.get('dual_status') or row.get('registration') or row.get('state')}"
        return "-"

    def next_action(self) -> str:
        state = self.progress.get("state")
        if state == "WAIT_PHYSICAL_SIM_SWAP":
            return "human SIM swap"
        if state == "COMPLETE":
            return "stop"
    ...
    def git_checkpoint(self, message: str) -> None:
        paths = [
            "tools/lte7_elisa_telia_crossover.py",
            "tools/ltap_lte7_crossover_watchdog.py",
    ...
    def run_phase(self, phase: str) -> None:
        ...
        while True:
            pending = [
                item for item in self.phase_items(phase)
                if self.progress["items"].setdefault(item["id"], {"state": "PENDING", "attempts": 0, "history": []}).get("state") not in base.TERMINAL_ITEM_STATES
            ]
            if not pending or STOP_FILE.exists():
                return
            self.run_loaded_item(pending[0])

    def run(self) -> None:
        self.start_heartbeat()
        try:
            if not self.verify_baseline():
                self.git_checkpoint("lte7 crossover: blocked baseline")
                return
            ...
            if not self.progress.get("phase_a_sim_map"):
                ...
                if not self.validate_one_elisa_one_telia(self.progress["phase_a_sim_map"]):
                    self.git_checkpoint("lte7 crossover: blocked sim mapping")
                    return
                self.update_matrix_summary()
                self.git_checkpoint("lte7 crossover: sim map verified")
            self.run_discovery()
            if not self.progress.get("selected_bands") and not self.progress.get("auto_fallback_selected"):
                ...
                self.save_progress()
                self.update_matrix_summary()
                self.git_checkpoint("lte7 crossover: no common bands")
                return
            ...
            if all(x.get("state") in base.TERMINAL_ITEM_STATES for x in self.progress["items"].values()):
                ...
                self.save_progress()
                self.update_matrix_summary()
                self.git_checkpoint("lte7 crossover: finalize")
        finally:
            self.close()
