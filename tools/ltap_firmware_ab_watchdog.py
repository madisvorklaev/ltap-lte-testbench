#!/usr/bin/env python3
"""Heartbeat watchdog logger for the FG621 firmware A/B campaign."""
...
def max_legitimate_stall_s(hb: dict[str, Any]) -> int:
    state = hb.get("campaign_state")
    if state in {"FIRMWARE_UPGRADING", "WAIT_FG621_REAPPEAR", "VERIFY_NEW_FIRMWARE"}:
        return 1500
    if hb.get("item_state") == "RUNNING":
        return 900
    if str(state or "").startswith("WAIT_"):
        return 1800
    return 420
